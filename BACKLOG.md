...
- **Prioridad**: Alta · **Esfuerzo**: M · **Estado**: Backlog
...
- **Criterios de aceptación**:
  - [ ] Test que crea la DB desde `version 1` y aplica todas las migraciones hasta la actual, verificando el esquema final (columnas y tablas esperadas).
  - [ ] Test de instalación limpia (`onCreate` directo en la última versión) que verifica que el esquema resultante es idéntico al de la ruta migrada.
  - [ ] Tests de los métodos CRUD más usados (`createExpense`, `guardarItemsGasto`, `filtrarNoVistas`) usando `sqflite_common_ffi` (ya está en `dev_dependencies`).
...
- **Criterios de aceptación**:
  - [ ] Test: oportunidades ya vistas no vuelven a notificarse.
  - [ ] Test: sin ticket o sin palabras clave configuradas, no se llama al API (`return 0` temprano).
  - [ ] Test: coincidencia de palabra clave case-insensitive y contra `nombre`+`descripcion`.
...
- **Criterios de aceptación**:
  - [ ] Workflow que corre en cada PR y push a `main`: `flutter pub get`, `flutter analyze`, `flutter test`.
  - [ ] El PR queda bloqueado (branch protection) si el workflow falla.
  - [ ] README actualizado con el badge de estado del workflow.
